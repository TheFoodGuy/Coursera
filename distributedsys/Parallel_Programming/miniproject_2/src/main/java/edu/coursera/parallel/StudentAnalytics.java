...
public final class StudentAnalytics {
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
        ...
        // this gets all of the studenets here that are inactive first
        // trying to avoid the issue of accidetnally adding them up into Map object
        Student[] inactiveStudents =  Stream.of(studentArray)
                .parallel()
                .filter(s -> !s.checkIsCurrent())
                .toArray(size -> new Student[size]);

        // Converting to Map object here and getting the values of each appearance
        Map<String, Long> mapInactiveStudents = Stream.of(inactiveStudents)
                .parallel()
                .map(s -> s.getFirstName())
                .collect(Collectors.groupingBy(Function.identity(), counting()));

        String result = mapInactiveStudents.entrySet()
                .parallelStream()
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(
            final Student[] studentArray) {
        Long numberOfFailedStudents = Stream.of(studentArray)
                .parallel()
                .filter(s -> (s.getGrade() < 65) && s.getAge() > 20 && !s.checkIsCurrent())
                .count();

        return (numberOfFailedStudents).intValue();
    }
}
